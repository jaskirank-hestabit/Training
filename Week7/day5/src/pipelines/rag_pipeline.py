...
def run_rag(query, filters=None, memory_context: str = "", temp_store=None):
    """
    ...
    Args:
        ...
        memory_context: Formatted conversation history string (from MemoryStore).
        temp_store:     Optional in-memory FAISSStore (for uploaded files).
                        If None, uses the default on-disk index.
    """
    retriever = HybridRetriever(store=temp_store)
    results   = retriever.retrieve(query=query, k=TOP_K, filters=filters)
    ...
    if memory_context.strip():
        memory_section = (
            "Conversation history (use only for context resolution, "
            "not as facts):\n" + memory_context + "\n\n"
        )
...
if __name__ == "__main__":
    q = input("Enter your query: ").strip() or "Explain how credit underwriting works"
    out = run_rag(q)
    print("\n====== ANSWER ======\n", out["answer"])
    print("\n====== SOURCES ======")
