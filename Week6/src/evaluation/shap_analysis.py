...
def shap_summary(model, X_train):
    ...
    plt.tight_layout()
    plt.savefig("src/evaluation/shap_summary.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_feature_importance(model, X_train):
    ...
    feat_imp.sort_values(ascending=False)[:15].plot.bar(figsize=(10, 5))
    plt.title("Feature Importance (top 15)")

    plt.tight_layout()
    plt.savefig("src/evaluation/feature_importance.png", dpi=150)
    plt.close()
...
def error_analysis(model, X_test, y_test):
    ...
    plt.tight_layout()
    plt.savefig("src/evaluation/error_heatmap.png", dpi=150)
    plt.close()
